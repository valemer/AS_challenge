...
int main(int argc, char** argv) {
    ...
    ros::Subscriber startSub = nh.subscribe("/start_points", 1000, startPointsCB);
    ros::Subscriber goalSub = nh.subscribe("/goal_points", 1000, goalPointsCB);

    // Publisher for the planned path
    plannedPathPub = nh.advertise<nav_msgs::Path>("/planned_path", 1000);
